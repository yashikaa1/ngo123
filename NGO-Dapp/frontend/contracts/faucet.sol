...
contract Faucet {
    ...
    function requestTokens(address receiver) public {
        require(
            msg.sender != address(0),
            "Request must not originate from a zero account"
        );
        require(receiver != address(0), "Reciever must not be a zero account");
        require(
            token.balanceOf(address(this)) >= withdrawalAmount,
            "Insufficient balance in faucet for withdrawal request"
        );
        require(
            block.timestamp >= nextAccessTime[receiver],
            "Insufficient time elapsed since last withdrawal - try again later."
        );

        nextAccessTime[receiver] = block.timestamp + lockTime;

        token.transfer(receiver, withdrawalAmount);
    }
